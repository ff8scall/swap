...
- **이미지 스타일 일관성**: NVIDIA NIM(FLUX.1) 엔진을 전수 사용하여 어두운 배경의 프리미엄 톤앤매너를 통일했습니다. 특정 어두운 피사체(코코아 등)의 렌더링 오류는 'Rim Lighting' 및 명칭 우회 전략을 통해 원본 스타일을 유지하며 해결했습니다.
- **플레이스홀더 원천 차단**: 데이터베이스의 'General Alt' 문구를 전수 제거했을 뿐만 아니라, UI 코드에서도 화이트리스트 기반의 명칭 렌더링을 적용하여 어색한 텍스트 노출을 이중으로 방지합니다.
